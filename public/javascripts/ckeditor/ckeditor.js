...
(function() {
    if (!window.CKEDITOR)window.CKEDITOR = (function() {
        var a = {timestamp:'A8LE4JO',version:'3.4.1',revision:'5892',_:{},status:'unloaded',basePath:(function() {
    ...
    (function() {
        ...
        function n(r) {
            var s = r.editor,t = r.data.path,u = s.config.useComputedState,v;
            u = u === undefined || u;
            if (u) {
                var w = s.getSelection(),x = w.getRanges();
            ...
            if (y != s.lang.dir)z.addClass('cke_mixed_dir_content'); else z.removeClass('cke_mixed_dir_content');
        }

        ;
        function o(r, s, t) {
            var u = r.getComputedStyle('direction'),v = r.getStyle('direction') || r.getAttribute('dir') || '';
            r.removeStyle('direction');
            if (v.toLowerCase() == s)r.removeAttribute('dir'); else r.setAttribute('dir', s);
            ...
            if (w != u) {
                ...
                while (z = y.next()) {
                    if (z.type == 1) {
                        ...
                        var A = z.getStyle('margin-right'),B = z.getStyle('margin-left');
        ...
        function q(r) {
            return function(s) {
                var t = s.getSelection(),u = s.config.enterMode,v = t.getRanges();
                if (v && v.length) {
                    ...
                    if (w) {
                        ...
                        var x = t.createBookmarks(),y,z;
                        for (var A = v.length - 1; A >= 0; A--) {
                            var B = [],C = new d.walker(v[A]);
        ...
        ;
        j.add('bidi', {requires:['styles','button'],init:function(r) {
            var s = function(u, v, w, x) {
                r.addCommand(w, new a.command(r, {exec:x}));
                r.ui.addButton(u, {label:v,command:w});
            },t = r.lang.bidi;
    ...
    (function() {
        ...
        var o = {canUndo:false,exec:c ? function(s) {
            ...
            switch (s.data.keyCode) {case 1000 + 86:case 2000 + 45:var t = this.document.getBody();if (!c && t.fire('beforepaste'))s.cancel(); else if (b.opera || b.gecko && b.version < 10900)t.fire('paste');return;case 1000 + 88:case 2000 + 46:var u = this;this.fire('saveSnapshot');setTimeout(function() {
                u.fire('saveSnapshot');
            }, 0);
            }
        };
    ...
    (function() {
        ...
        j.add('dialogadvtab', {createAdvancedTab:function(n, o) {
            ...
            if (o.id || o.dir) {
                ...
                if (o.dir)r.push({id:'advLangDir',att:'dir',type:'select',label:p.langDir,'default':'',style:'width:100%',items:[
                    [p.notSet,''],
                    [p.langDirLTR,'ltr'],
                    [p.langDirRTL,'rtl']
                ],setup:l,commit:m});
            ...
            if (o.styles || o.classes) {
                r = [];
                if (o.styles)r.push({id:'advStyles',att:'style',type:'text',label:p.styles,'default':'',onChange:function() {
    ...
    a._.elementsPath = {click:function(l, m) {
        ...
        switch (n.getKeystroke()) {case s ? 39 : 37:case 9:r = a.document.getById(q + (m + 1));if (!r)r = a.document.getById(q + '0');r.focus();return false;case s ? 37 : 39:case 2000 + 9:r = a.document.getById(q + (m - 1));if (!r)r = a.document.getById(q + (p._.elementsPath.list.length - 1));r.focus();return false;case 27:p.focus();return false;case 13:case 32:this.click(l, m);return false;
        }
        return true;
    }};
    (function() {
    ...
    (function() {
        ...
        function p(q, r) {
            ...
            if (!r) {
                q = q.split(',');
                var v = document.createElement('div'),w;
                v.innerHTML = '&' + q.join(';&') + ';';
                w = v.innerHTML;
                v = null;
        ...
        ;
        j.add('entities', {afterInit:function(q) {
            var r = q.config,s = q.dataProcessor,t = s && s.htmlFilter;
            if (t) {
                var u = l;
                if (r.entities) {
                    u += ',' + m;
                ...
                ;
                var y = p([l,'shy'].join(','), true),z = new RegExp(y.regex, 'g');
    ...
    j.add('forms', {init:function(l) {
        ...
        l.on('doubleclick', function(p) {
            ...
            if (q.is('form'))p.data.dialog = 'form'; else if (q.is('select'))p.data.dialog = 'select'; else if (q.is('textarea'))p.data.dialog = 'textarea'; else if (q.is('img') && q.getAttribute('_cke_real_element_type') == 'hiddenfield')p.data.dialog = 'hiddenfield'; else if (q.is('input')) {
                ...
                switch (r) {case 'text':case 'password':p.data.dialog = 'textfield';break;case 'button':case 'submit':case 'reset':p.data.dialog = 'button';break;case 'checkbox':p.data.dialog = 'checkbox';break;case 'radio':p.data.dialog = 'radio';break;case 'image':p.data.dialog = 'imagebutton';break;
                }
            }
        });
    ...
    (function() {
        ...
        function q(R) {
            o(R, true);
            if (p(R))if (c)R.add(new a.htmlParser.text('\xa0')); else R.add(new a.htmlParser.element('br', {}));
        }
        ...
        var x = {elementNames:[
            ...
            if (R.substr(0, m.length) == m) {
                if (R.substr(m.length, 3) == '{C}')R = R.substr(m.length + 3); else R = R.substr(m.length);
        ...
        var B = /<((?:a|area|img|input)[\s\S]*?\s)((href|src|name)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+)))([^>]*)>/gi,C = /\s_cke_saved_src\s*=/,D = /(?:<style(?=[ >])[^>]*>[\s\S]*<\/style>)|(?:<(:?link|meta|base)[^>]*>)/gi,E = /<cke:encoded>([^<]*)<\/cke:encoded>/gi,F = /(<\/?)((?:object|embed|param|html|body|head|title)[^>]*>)/gi,G = /(<\/?)cke:((?:html|body|head|title)[^>]*>)/gi,H = /<cke:(param|embed)([^>]*?)\/?>(?!\s*<\/cke:\1)/gi;

        function I(R) {
            return R.replace(B, function(S, T, U, V, W) {
                if (V == 'src' && C.test(S))return S; else return '<' + T + U + ' _cke_saved_' + U + W + '>';
            });
        }
    ...
    (function() {
        ...
        q.prototype = {exec:function(t) {
            ...
            function w(N) {
                var O = C.startContainer,P = C.endContainer;
                ...
                var V = u.name == 'indent' ? 1 : -1,W = R[0],X = R[R.length - 1],Y = j.list.listToArray(N, v),Z = Y[X.getCustomData('listarray_index')].indent;
                ...
                var ab = N.getAttribute('dir') || N.getStyle('direction'),ac = j.list.arrayToList(Y, v, null, t.config.enterMode, ab);
                if (u.name == 'outdent') {
                    var ad;
                    if ((ad = N.getParent()) && ad.is('li')) {
                        var ae = ac.listNode.getChildren(),af = [],ag = ae.count(),ah;
                        for (U = ag - 1; U >= 0; U--) {
            ...
            ;
            function x() {
                var N = C.createIterator(),O = t.config.enterMode;
            ...
            function y(N) {
                ...
                if (u.useIndentClasses) {
                    var O = N.$.className.match(u.classNameRegex),P = 0;
                    if (O) {
                        O = O[1];
                    ...
                    if (P < 1)N.$.className = Q; else N.addClass(t.config.indentClasses[P - 1]);
                } else {
                    var R = r(N),S = parseInt(N.getStyle(R), 10);
                    if (isNaN(S))S = 0;
            ...
            ;
            var z = t.getSelection(),A = z.createBookmarks(true),B = z && z.getRanges(true),C,D = function(N) {
            ...
            while (C = E.getNextRange()) {
                ...
                var F = C.startContainer,G = C.endContainer,H = C.getCommonAncestor(),I = H;
                ...
                if (I) {
                    ...
                    }),L = C.startContainer,M = K.equals(L) || K.contains(L);
                    if (!(M && y(I)))w(I);
                } else x();
            }
            h.clearAllMarkers(v);
    ...
    (function() {
        ...
        function m(p, q) {
            q = q === undefined || q;
            var r;
            if (q)r = p.getComputedStyle('text-align'); else {
        ...
        function o(p, q, r) {
            ...
            if (s) {
                switch (r) {case 'left':t.cssClassName = s[0];break;case 'center':t.cssClassName = s[1];break;case 'right':t.cssClassName = s[2];break;case 'justify':t.cssClassName = s[3];break;
                }
    ...
    (function() {
        ...
        function r(B, C, D) {
            ...
            while (G.length) {
                ...
                if (P.is('pre') || q.test(P.getName()))P.appendTo(Q); else {
    ...
    j.add('pagebreak', {init:function(l) {
        ...
        if (n)n.addRules({elements:{div:function(o) {
            ...
            if (s && /page-break-after\s*:\s*always/i.test(q) && /display\s*:\s*none/i.test(s)) {
                var t = l.createFakeParserElement(o, 'cke_pagebreak', 'div'),u = l.lang.pagebreakAlt;
                t.attributes.alt = u;
    ...
    j.pagebreakCmd = {exec:function(l) {
        var m = l.lang.pagebreakAlt,n = h.createFromHtml('<div style="page-break-after: always;"><span style="display: none;">&nbsp;</span></div>');
    ...
    j.add('popup');
    e.extend(a.editor.prototype, {popup:function(l, m, n, o) {
        m = m || '80%';
        n = n || '70%';
        ...
        if (m < 640)m = 640;
        if (n < 420)n = 420;
        var p = parseInt((window.screen.height - n) / 2, 10),q = parseInt((window.screen.width - m) / 2, 10);
    ...
    (function() {
        ...
        j.add('showblocks', {requires:['wysiwygarea'],init:function(q) {
            ...
            q.ui.addButton('ShowBlocks', {label:q.lang.showBlocks,command:'showblocks'});
            q.on('mode', function() {
    ...
    (function() {
        j.add('stylescombo', {requires:['richcombo','styles'],init:function(m) {
            ...
            m.ui.addRichCombo('Styles', {label:o.label,title:o.panelTitle,className:'cke_styles',panel:{css:m.skin.editor.css.concat(n.contentsCss),multiSelect:true,attributes:{'aria-label':o.panelTitle}},init:function() {
                ...
                if (t.type == 2 && t.checkActive(v))t.remove(m.document); else if (t.type == 3 && t.checkActive(v))t.remove(m.document); else t.apply(m.document);
                m.fire('saveSnapshot');
            },onRender:function() {
    ...
    i.undoStackSize = 20;
    (function() {
        var l = {table:1,pre:1},m = /\s*<(p|div|address|h\d|center)[^>]*>\s*(?:<br[^>]*>|&nbsp;|\u00A0|&#160;)?\s*(:?<\/\1>)?(?=\s*$|<\/body>)/gi,n = d.walker.whitespaces(true);
        ...
        function y(A) {
            ...
            if (!E || E == 1 && D.getFirst().hasAttribute('_moz_editor_bogus_node')) {
                r(A);
                var F = A.element.getDocument(),G = F.getDocumentElement(),H = G.$.scrollTop,I = G.$.scrollLeft,J = C.$.createEvent('KeyEvents');
        ...
        j.add('wysiwygarea', {requires:['editingblock'],init:function(A) {
            ...
            A.on('editingBlockReady', function() {
                var J,K,L,M,N,O,P = b.isCustomDomain(),Q = function(T) {
                    ...
                    var V = A.element,W = b.gecko && !V.isVisible(),X = {};
                    if (W) {
                        V.show();
                        X = {position:V.getStyle('position'),top:V.getStyle('top')};
                        V.setStyles({position:'absolute',top:'-3000px'});
                    }
                    J.append(K);
    ...
    (function() {
        ...
        function m() {
            var R = this;
            var N = R._.currentTabId,O = R._.tabIdList.length,P = e.indexOf(R._.tabIdList, N) + O;
        ...
        function n() {
            var R = this;
            var N = R._.currentTabId,O = R._.tabIdList.length,P = e.indexOf(R._.tabIdList, N);
            for (var Q = P + 1; Q < P + O; Q++) {
                if (l.call(R, R._.tabIdList[Q % O]))return R._.tabIdList[Q % O];
            }
            return null;
        ...
        a.dialog = function(N, O) {
            ...
            P = e.clone(P);
            P = new u(this, P);
            var Q = a.document,R = N.theme.buildDialog(N);
            this._ = {editor:N,element:R.element,name:O,contentSize:{width:0,height:0},size:{width:0,height:0},updateSize:false,contents:{},buttons:{},accessKeyMap:{},tabs:{},tabIdList:[],currentTabId:null,currentTabIndex:null,pageCount:0,lastTab:null,tabBarMode:false,focusList:[],currentFocusIndex:0,hasFocus:false};
            this.parts = R.parts;
            e.setTimeout(function() {
            ...
            a.event.call(this);
            this.definition = P = a.fire('dialogDefinition', {name:O,definition:P}, N).definition;
            ...
            var S = this,T = function(ae) {
                var af = S._.contents,ag = false;
            ...
            this.on('ok', function(ae) {
                T(function(af) {
                    if (af.validate) {
                        ...
                        if (ag === false) {
                            if (af.select)af.select(); else af.focus();
                            ae.data.hide = false;
                            ae.stop();
                            return true;
                        }
            ...
            function U() {
                ...
                ae.sort(function(ah, ai) {
                    if (ah.tabIndex != ai.tabIndex)return ai.tabIndex - ah.tabIndex; else return ah.focusIndex - ai.focusIndex;
                });
                var af = ae.length;
            ...
            ;
            function V(ae) {
                var af = S._.focusList,ag = ae ? 1 : -1;
                ...
                var ai = (ah + ag + af.length) % af.length,aj = ai;
            ...
            function X(ae) {
                ...
                var af = ae.data.getKeystroke(),ag = N.lang.dir == 'rtl';
                W = 0;
                if (af == 9 || af == 2000 + 9) {
                    var ah = af == 2000 + 9;
                    if (S._.tabBarMode) {
            ...
            this.on('show', function() {
                ...
                if (N.config.dialog_startupFocusTab && S._.pageCount > 1) {
                    ...
                    if (ai._.editor.mode == 'wysiwyg' && c) {
                        var af = N.document.$.selection,ag = af.createRange();
            ...
            if (b.ie6Compat)this.on('load', function(ae) {
                var af = this.getElement(),ag = af.getFirst();
                ag.remove();
                ag.appendTo(af);
            }, this);
            w(this);
            ...
            var ac = [],ad = a.dialog._.uiElementBuilders.hbox.build(this, {type:'hbox',className:'cke_dialog_footer_buttons',widths:[],children:P.buttons}, ac).getChild();
        ...
        function p(N, O, P) {
            ...
            O.on('keydown', function(Q) {
                if (Q.data.getKeystroke() in {32:1,13:1})this.fire('click');
            });
            O.on('focus', function() {
                this.fire('mouseover');
            });
        ...
        a.dialog.prototype = {destroy:function() {
            ...
            return function(N, O) {
                ...
                a.dialog.fire('resize', {dialog:P,skin:P._.editor.skinName,width:N,height:O}, P._.editor);
                P._.contentSize = {width:N,height:O};
                P._.updateSize = true;
            };
        })(),getSize:function() {
            var P = this;
            if (!P._.updateSize)return P._.size;
            var N = P._.element.getFirst(),O = P._.size = {width:N.$.offsetWidth || 0,height:N.$.offsetHeight || 0};
            ...
            return function(O, P) {
                ...
                S._.position = {x:O,y:P};
                if (!N) {
                    var R = a.document.getWindow().getScrollPosition();
                    O += R.x;
                    P += R.y;
                }
                Q.setStyles({left:(O > 0 ? O : 0) + 'px',top:(P > 0 ? P : 0) + 'px'});
            };
            ...
            if (a.dialog._.currentTop === null) {
                ...
                for (var S in {keyup:1,keydown:1,keypress:1})P.on(S, M);
            } else {
            ...
            e.setTimeout(function() {
                var U = a.document.getWindow().getViewPaneSize(),V = this.getSize();
            ...
            if (!this._.parentDialog) {
                ...
                for (var P in {keyup:1,keydown:1,keypress:1})N.removeListener(P, M);
                var Q = this._.editor;
                Q.focus();
                if (Q.mode == 'wysiwyg' && c) {
            ...
            var Z = this;
            var O = [],P = N.label ? ' title="' + e.htmlEncode(N.label) + '"' : '',Q = N.elements,R = a.dialog._.uiElementBuilders.vbox.build(Z, {type:'vbox',className:'cke_dialog_page_contents',children:N.elements,expand:!!N.expand,padding:N.padding,style:N.style || 'width: 100%; height: 100%;'}, O),S = h.createFromHtml(O.join(''));
            S.setAttribute('role', 'tabpanel');
            var T = b,U = 'cke_' + N.id + '_' + e.getNextNumber(),V = h.createFromHtml(['<a class="cke_dialog_tab"',Z._.pageCount > 0 ? ' cke_last' : 'cke_first',P,!!N.hidden ? ' style="display:none"' : '',' id="',U,'"',T.gecko && T.version >= 10900 && !T.hc ? '' : ' href="javascript:void(0)"',' tabIndex="-1"',' hidefocus="true"',' role="tab">',N.label,'</a>'].join(''));
            ...
            Z._.lastTab = V;
            Z.updateStyle();
            var W = Z._.contents[N.id] = {},X,Y = R.getChild();
            while (X = Y.shift()) {
                W[X.id] = X;
            ...
            if (this.fire('selectPage', {page:N,currentPage:this._.currentTabId}) === true)return;
            for (var O in this._.tabs) {
                var P = this._.tabs[O][0],Q = this._.tabs[O][1];
                if (O != N) {
            ...
            if (!O || P._.pageCount == 1)return; else if (N == P._.currentTabId)P.selectPage(m.call(P));
            O.hide();
            P._.pageCount--;
            P.updateStyle();
        },showPage:function(N) {
        ...
        e.extend(a.dialog, {add:function(N, O) {
            ...
            var N = function(O, P) {
                P = P || {};
                return e.extend({id:'ok',type:'button',label:O.lang.common.ok,'class':'cke_dialog_ui_button_ok',onClick:function(Q) {
                    var R = Q.data.dialog;
                    if (R.fire('ok', {hide:true}).hide !== false)R.hide();
                }}, P, true);
            };
            N.type = 'button';
            ...
            var N = function(O, P) {
                P = P || {};
                return e.extend({id:'cancel',type:'button',label:O.lang.common.cancel,'class':'cke_dialog_ui_button_cancel',onClick:function(Q) {
                    var R = Q.data.dialog;
                    if (R.fire('cancel', {hide:true}).hide !== false)R.hide();
                }}, P, true);
            };
            N.type = 'button';
            N.override = function(O) {
        ...
        var q = {resizable:3,minWidth:600,minHeight:400,buttons:[a.dialog.okButton,a.dialog.cancelButton]};
        ...
        u.prototype = {getContents:function(N) {
        ...
        function w(N) {
            var O = null,P = null,Q = N.getElement().getFirst(),R = N.getParentEditor(),S = R.config.dialog_magnetDistance,T = R.skin.margins || [0,0,0,0];
            if (typeof S == 'undefined')S = 20;
            function U(W) {
                var X = N.getSize(),Y = a.document.getWindow().getViewPaneSize(),Z = W.data.$.screenX,aa = W.data.$.screenY,ab = Z - O.x,ac = aa - O.y,ad,ae;
                O = {x:Z,y:aa};
                P.x += ab;
                P.y += ac;
                if (P.x + T[3] < S)ad = -T[3]; else if (P.x - T[1] > Y.width - X.width - S)ad = Y.width - X.width + T[1]; else ad = P.x;
                if (P.y + T[0] < S)ae = -T[0]; else if (P.y - T[2] > Y.height - X.height - S)ae = Y.height - X.height + T[2]; else ae = P.y;
                N.move(ad, ae);
            ...
            N.parts.title.on('mousedown', function(W) {
                N._.updateSize = true;
                O = {x:W.data.$.screenX,y:W.data.$.screenY};
        ...
        ;
        function x(N) {
            var O = N.definition,P = O.minWidth || 0,Q = O.minHeight || 0,R = O.resizable,S = N.getParentEditor().skin.margins || [0,0,0,0];
            ...
            ;
            var X = null,Y = null,Z = N._.editor.config.magnetDistance,aa = ['tl','t','tr','l','r','bl','b','br'];

            function ab(ae) {
                var af = ae.listenerData.part,ag = N.getSize();
                Y = N.getPosition();
                e.extend(Y, {x2:Y.x + ag.width,y2:Y.y + ag.height});
                X = {x:ae.data.$.screenX,y:ae.data.$.screenY};
                a.document.on('mousemove', ac, N, {part:af});
                a.document.on('mouseup', ad, N, {part:af});
                if (b.ie6Compat) {
                    var ah = A.getChild(0).getFrameDocument();
                    ah.on('mousemove', ac, N, {part:af});
                    ah.on('mouseup', ad, N, {part:af});
                }
                ae.data.preventDefault();
            }
            ...
            ;
            function ac(ae) {
                var af = ae.data.$.screenX,ag = ae.data.$.screenY,ah = af - X.x,ai = ag - X.y,aj = a.document.getWindow().getViewPaneSize(),ak = ae.listenerData.part;
                ...
                X = {x:af,y:ag};
                var al,am,an,ao;
                if (Y.x + S[3] < Z)al = -S[3]; else if (ak.search('l') != -1 && Y.x2 - Y.x < P + Z)al = Y.x2 - P; else al = Y.x;
                if (Y.y + S[0] < Z)am = -S[0]; else if (ak.search('t') != -1 && Y.y2 - Y.y < Q + Z)am = Y.y2 - Q; else am = Y.y;
                if (Y.x2 - S[1] > aj.width - Z)an = aj.width + S[1]; else if (ak.search('r') != -1 && Y.x2 - Y.x < P + Z)an = Y.x + P; else an = Y.x2;
                if (Y.y2 - S[2] > aj.height - Z)ao = aj.height + S[2]; else if (ak.search('b') != -1 && Y.y2 - Y.y < Q + Z)ao = Y.y + Q; else ao = Y.y2;
                N.move(al, am);
        ...
        function B(N) {
            var O = a.document.getWindow(),P = N.config.dialog_backgroundCoverColor || 'white',Q = N.config.dialog_backgroundCoverOpacity,R = N.config.baseFloatZIndex,S = e.genKey(P, Q, R),T = z[S];
            if (!T) {
                ...
                if (b.ie6Compat) {
                    var V = b.isCustomDomain(),W = "<html><body style=\\'background-color:" + P + ";\\'></body></html>";
            ...
            var X = function() {
                ...
                T.setStyles({width:aa.width + 'px',height:aa.height + 'px'});
            },Y = function() {
                var aa = O.getScrollPosition(),ab = a.dialog._.currentTop;
                T.setStyles({left:aa.x + 'px',top:aa.y + 'px'});
                do{
        ...
        ;
        var E = {},F = function(N) {
            var O = N.data.$.ctrlKey || N.data.$.metaKey,P = N.data.$.altKey,Q = N.data.$.shiftKey,R = String.fromCharCode(N.data.$.keyCode),S = E[(O ? 'CTRL+' : '') + (P ? 'ALT+' : '') + (Q ? 'SHIFT+' : '') + R];
            ...
            var S = E[P] || (E[P] = []);
            S.push({uiElement:N,dialog:O,key:P,keyup:R || N.accessKeyUp,keydown:Q || N.accessKeyDown});
        },I = function(N) {
            for (var O in E) {
                var P = E[O];
        ...
        },L = {27:1,13:1},M = function(N) {
            if (N.data.getKeystroke() in L)N.data.stopPropagation();
        };
        (function() {
            k.dialog = {uiElement:function(N, O, P, Q, R, S, T) {
                if (arguments.length < 4)return;
                var U = (Q.call ? Q(O) : Q) || 'div',V = ['<',U,' '],W = (R && R.call ? R(O) : R) || {},X = (S && S.call ? S(O) : S) || {},Y = (T && T.call ? T.call(this, N, O) : T) || '',Z = this.domId = X.id || e.getNextId() + '_uiElement',aa = this.id = O.id,ab;
                X.id = Z;
                var ac = {};
                ...
                if (arguments.length < 4)return;
                this._ || (this._ = {});
                var S = this._.children = O,T = R && R.widths || null,U = R && R.height || null,V = {},W,X = function() {
                    var Z = ['<tbody><tr class="cke_dialog_ui_hbox">'];
                    for (W = 0; W < P.length; W++) {
                        var aa = 'cke_dialog_ui_hbox_child',ab = [];
                ...
                k.dialog.uiElement.call(this, N, R || {type:'hbox'}, Q, 'table', V, Y, X);
            },vbox:function(N, O, P, Q, R) {
                if (arguments.length < 3)return;
                this._ || (this._ = {});
                var S = this._.children = O,T = R && R.width || null,U = R && R.heights || null,V = function() {
                    ...
                    for (var X = 0; X < P.length; X++) {
                        ...
                        if (U)Y.push('height:' + e.cssLength(U[X])); else if (R && R.expand)Y.push('height:' + Math.floor(100 / P.length) + '%');
                ...
                k.dialog.uiElement.call(this, N, R || {type:'vbox'}, Q, 'div', null, {role:'presentation'}, V);
            }};
        })();
        k.dialog.uiElement.prototype = {getElement:function() {
            ...
            var O = /^on([A-Z]\w+)/,P,Q = function(S, T, U, V) {
                T.on('load', function() {
            ...
            for (var R in N) {
                ...
                if (this.eventProcessors[R])this.eventProcessors[R].call(this, this._.dialog, N[R]); else Q(this, this._.dialog, P[1].toLowerCase(), N[R]);
            }
            return this;
        },eventProcessors:{onLoad:function(N, O) {
            N.on('load', O, this);
        },onShow:function(N, O) {
            N.on('show', O, this);
        },onHide:function(N, O) {
        ...
        k.dialog.hbox.prototype = e.extend(new k.dialog.uiElement(), {getChild:function(N) {
            var O = this;
            if (arguments.length < 1)return O._.children.concat();
            if (!N.splice)N = [N];
            if (N.length < 2)return O._.children[N[0]]; else return O._.children[N[0]] && O._.children[N[0]].getChild ? O._.children[N[0]].getChild(N.slice(1, N.length)) : null;
        }}, true);
        k.dialog.vbox.prototype = new k.dialog.hbox();
        (function() {
            var N = {build:function(O, P, Q) {
                var R = P.children,S,T = [],U = [];
        ...
        a.dialogCommand.prototype = {exec:function(N) {
        ...
        (function() {
            ...
            a.dialog.validate = {functions:function() {
                return function() {
                    var W = this;
                    var Q = W && W.getValue ? W.getValue() : arguments[0],R = undefined,S = 2,T = [],U;
                    for (U = 0; U < arguments.length; U++) {
                        if (typeof arguments[U] == 'function')T.push(arguments[U]); else break;
                    }
                    ...
                    for (U = 0; U < T.length; U++) {
                        if (S == 2)V = V && T[U](Q); else V = V || T[U](Q);
                    }
                    if (!V) {
                ...
                return function() {
                    ...
                    if (!Q.test(S)) {
                        if (R !== undefined)alert(R);
                        if (T && (T.select || T.focus))if (T.select)T.select(); else T.focus();
                        return false;
                    }
                    return true;
                };
    ...
    (function() {
        ...
        function o(P) {
            ...
            var S = al.element,T = al._.definition,U,V = f[S] || (U = true,f.span);
            P.enlarge(1);
            P.trim();
            var W = P.createBookmark(),X = W.startNode,Y = W.endNode,Z = X,aa;
            while (Z) {
                var ab = false;
                if (Z.equals(Y)) {
                    Z = null;
                    ...
                    var ac = Z.type,ad = ac == 1 ? Z.getName() : null;
                ...
                if (ab && aa && !aa.collapsed) {
                    var ah = E(al, Q),ai = aa.getCommonAncestor();
                    while (ah && ai) {
                        if (ai.getName() == S) {
        ...
        function p(P) {
            ...
            if (P.collapsed) {
                ...
                if (T) {
                    ...
                    for (U = 0; true; U++) {
                        var Y = S.elements[U];
                        if (Y.equals(T))break; else if (Y.match)continue; else Y = Y.clone();
                        Y.append(X);
                        X = Y;
                    }
                ...
                var Z = Q.endNode,aa = this;

                function ab() {
                    var ae = new d.elementPath(R.getParent()),af = new d.elementPath(Z.getParent()),ag = null,ah = null;
                ...
                while (!ac.equals(Z)) {
                    ...
                    if (ac.type == 1 && this.checkElementRemovable(ac)) {
                        if (ac.getName() == this.element)A(this, ac); else C(ac, J(this)[ac.getName()]);
        ...
        function r(P) {
            var Q = P.getCommonAncestor(true, true),R = Q.getAscendant(this.element, true);
            if (!R)return;
            var S = this,T = S._.definition,U = T.attributes,V = a.style.getStyleText(T);
        ...
        function t(P, Q) {
            ...
            if (T)Q = z(P, Q); else if (U)Q = y(w(P), Q); else P.moveChildren(Q);
            Q.replace(P);
            if (R)v(Q);
        }
        ...
        ;
        function w(P) {
            var Q = /(\S\s*)\n(?:\s|(<span[^>]+_cke_bookmark.*?\/span>))*\n(?!$)/gi,R = P.getName(),S = x(P.getOuterHtml(), Q, function(U, V, W) {
                return V + '</pre>' + W + '<pre>';
            }),T = [];
        ...
        ;
        function O(P, Q) {
            var R = P.getSelection(),S = R.createBookmarks(),T = R.getRanges(true),U = Q ? this.removeFromRange : this.applyToRange,V,W = T.createIterator();
    ...
    (function() {
        ...
        l.prototype = {getNextParagraph:function(o) {
            ...
            if (!P._.lastNode) {
                ...
                var v = new d.walker(q),w = d.walker.bookmark(true, true);
                v.evaluator = w;
                P._.nextNode = v.next();
                v = new d.walker(q);
                v.evaluator = w;
                ...
                if (P._.lastNode && P._.lastNode.type == 3 && !e.trim(P._.lastNode.getText()) && P._.lastNode.getParent().isBlockBoundary()) {
                    ...
                    if (y.checkEndOfBlock()) {
                        var z = new d.elementPath(y.endContainer),A = z.block || z.blockLimit;
            ...
            P._.nextNode = null;
            while (B) {
                var C = false,D = B.hasAscendant('pre'),E = B.type != 1,F = false;
                if (!E) {
                    var G = B.getName();
                    if (B.isBlockBoundary(P.forceBrBreak && !D && {br:1})) {
                        if (G == 'br')E = true; else if (!q && !B.getChildCount() && G != 'hr') {
                            p = B;
                            r = B.equals(x);
                            break;
                        }
                ...
                if (q && !C)while (!B.getNext() && !r) {
                    var H = B.getParent();
                    if (H.isBlockBoundary(P.forceBrBreak && !D && {br:1})) {
                        C = true;
                        r = r || H.equals(x);
                        break;
                    }
            ...
            if (!p) {
                ...
                var I = new d.elementPath(q.startContainer),J = I.blockLimit,K = {div:1,th:1,td:1};
                p = I.block;
                if (!p && !P.enforceRealBlocks && K[J.getName()] && q.checkStartOfBlock() && q.checkEndOfBlock())p = J; else if (!p || P.enforceRealBlocks && p.getName() == 'li') {
            ...
            if (t) {
                var M = p.getPrevious();
                if (M && M.type == 1)if (M.getName() == 'br')M.remove(); else if (M.getLast() && M.getLast().$.nodeName.toLowerCase() == 'br')M.getLast().remove();
            }
            if (u) {
                var N = d.walker.bookmark(false, true),O = p.getLast();
        ...
        k.floatPanel = e.createClass({$:function(o, p, q, r) {
            ...
            e.setTimeout(function() {
                ...
                var D = e.bind(function() {
                    ...
                    if (c) {
                        var N = new h(v.$.offsetParent),O = N;
                        if (O.getName() == 'html')O = O.getDocument().getBody();
                        if (O.getComputedStyle('direction') == 'rtl')if (b.ie8Compat)A -= v.getDocument().getDocumentElement().$.scrollLeft * 2; else A -= N.$.scrollWidth - N.$.clientWidth;
                    }
        ...
        a.menu = e.createClass({$:function(m, n) {
            ...
            if (!s) {
                ...
                this._.itemClickFn = e.addFunction(function(E) {
                    ...
                    if (F.getItems)G._.showSubMenu(E); else G.onClick && G.onClick(F);
                }, this);
            }
            l(q);
            var x = r.container.getChild(1),y = x.hasClass('cke_mixed_dir_content') ? ' cke_mixed_dir_content' : '',z = ['<div class="cke_menu' + y + '" role="presentation">'],A = q.length,B = A && q[0].group;
            for (var C = 0; C < A; C++) {
                var D = q[C];
                if (B != D.group) {
    ...
    (function() {
        ...
        var q = {modes:{wysiwyg:1,source:1},exec:function(s) {
            switch (s.mode) {case 'wysiwyg':s.document.$.execCommand('SelectAll', false, null);break;case 'source':var t = s.textarea.$;if (c)t.createTextRange().execCommand('SelectAll'); else {
                t.selectionStart = 0;
        ...
        j.add('selection', {init:function(s) {
            s.on('contentDom', function() {
                ...
                if (c) {
                    ...
                    function B(C) {
                        if (x) {
                            var D = s.document,E = s.getSelection(),F = E && E.getNative();
                            if (C && F && F.type == 'None')if (!D.$.queryCommandEnabled('InsertImage')) {
                                e.setTimeout(B, 50, this, true);
                                return;
                            }
                            var G;
                            if (F && F.type && F.type != 'Control' && (G = F.createRange()) && (G = G.parentElement()) && (G = G.nodeName) && G.toLowerCase() in {input:1,textarea:1})return;
                            w = F && E.getRanges()[0];
                            o.call(s);
                        }
                    }
        ...
        d.selection.prototype = {getNative:c ? function() {
            ...
            var s = c ? (function() {
                var t = function(u, v) {
                    ...
                    for (var z = 0; z < x.length; z++) {
                        ...
                        if (A.nodeType == 1) {
                            ...
                            y.collapse();
                            if (B > 0)break; else if (!B || C == 1 && B == -1)return{container:w,offset:z}; else if (!C)return{container:w,offset:z + 1};
                            y = null;
                        }
                    }
                    if (!y) {
                ...
                var t = [],u,v = this.document,w = this.getNative();
                if (!w)return t;
                if (!w.rangeCount) {
                    u = new d.range(v);
    ...
    a.editor.prototype.resize = function(l, m, n, o) {
        var p = this.container,q = a.document.getById('cke_contents_' + this.name),r = o ? p.getChild(1) : p;
